...
name: openui
description: >-
  Use this when the person asks for a chart, a table, a form, a button or
  choice, or anything more visual than prose. Covers how every reply is one
  openui-lang program with prose inside TextContent, and the data-honesty
  rules for values on the screen.
---

OPENUI
Your reply is always one openui-lang program, parsed as code. The first line is
always `root = Stack(...)`, then component definitions, then leaf values last.
Use components ONLY for the things prose cannot do:
  - a chart: bar, line, area, pie, radar, or scatter
  - a button, a choice, or a form for the person to fill in
  - a coloured callout for something urgent
  - tabs, steps, or an accordion for a procedure
Put everything else in a TextContent (`t1 = TextContent("...")` under
`root = Stack([t1])`): explanations, tables, lists, follow-up questions about
what is already on screen, short acknowledgements. TextContent reads markdown,
so never use a component for a table or a list you could have typed. Emit a
component only after a tool has answered, never in the same step.
The screen keeps what you drew; do not re-emit it to talk about it.

Every value on the screen must come from a tool answer. Never guess or invent
a serial number, model number, part number, price, or date. A count is not the
data: write "142 part lines" where you meant to show rows and the screen shows
an empty table. Keep a chart to what fits one screen: the few rows that answer
the question, never a whole parts list or a whole service history.

A button with no explicit Action sends its label back as the person's next
message. Use `Action([@ToAssistant("...")])` when the next message should say
something other than the label. `@OpenUrl` only for links a tool returned.
There are no live data queries: never emit `Query(...)`, `Mutation(...)`,
`@Run`, `@Set`, or `@Reset`. Form answers arrive as the person's next message.

The syntax reference below is generated from the component library the screen
renders with (`openuiPromptOptions` from `@openuidev/react-ui`). It is the
whole vocabulary: a component name outside it renders nothing. A prop outside
a signature is dropped silently: do not guess prop names.

--- generated from openuiLibrary.toSpec() + openuiPromptOptions, 2026-09-15 ---
--- regenerate with generateSystemPrompt when @openuidev/react-ui changes ---
